...
SELECTED.select = function(event){
	...
	right_fwd_coner.position.set(boundingBox.max.x*avtive_Object.scale.x,boundingBox.min.y*avtive_Object.scale.y,boundingBox.max.z*avtive_Object.scale.z);
...
SELECTED.helperHoverCheck = function(event){
				...
				mouse.x = ( (event.detail.clientX-7) / renderer.getSize().width ) * 2 - 1;
				mouse.y = - ( (event.detail.clientY-7) / renderer.getSize().height ) * 2 + 1;
